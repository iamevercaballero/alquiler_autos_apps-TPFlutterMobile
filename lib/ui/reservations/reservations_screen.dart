...
class ReservationsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      body: Consumer<ReservationController>(
        builder: (_, c, __) => ListView(
          children: [
            ...
            for (final r in c.activas)
              ListTile(
                leading: const Icon(Icons.event_available),
                title: Text('Reserva ${r.idReserva}'),
                subtitle: Text(
                  'Cliente: ${r.idCliente} • Vehículo: ${r.idVehiculo}\n'
                  '${fmtDate(r.fechaInicio)} → ${fmtDate(r.fechaFin)}',
                ),
              ),
            const Divider(),
            const ListTile(title: Text('Histórico')),
            for (final r in c.historico)
              ListTile(
                leading: const Icon(Icons.history),
                title: Text('Reserva ${r.idReserva}'),
                subtitle: Text(
                  'Cliente: ${r.idCliente} • Vehículo: ${r.idVehiculo}\n'
                  'Inicio: ${fmtDate(r.fechaInicio)} • '
                  'Entrega: ${r.fechaEntregaReal != null ? fmtDate(r.fechaEntregaReal!) : '-'}',
                ),
              ),
          ],
        ),
      ...
      // 👉 FAB visible siempre para crear reserva
              ...
              DropdownButtonFormField<String>(
                ...
                items: [
                  for (final c in clientes)
                    DropdownMenuItem(
                      value: c.idCliente,
                      child: Text('${c.nombre} ${c.apellido} (${c.idCliente})'),
                    ),
                ],
              ...
              DropdownButtonFormField<String>(
                ...
                items: [
                  for (final v in disponibles)
                    DropdownMenuItem(
                      value: v.idVehiculo,
                      child: Text('${v.marca} ${v.modelo} (${v.idVehiculo})'),
                    ),
                ],
        ...
        ElevatedButton(
          onPressed: () {
            ...
            if (fin.isBefore(inicio)) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('La fecha de fin no puede ser anterior al inicio')),
              );
              return;
            }
            final r = Reservation(
